    ...
    <div class="container py-4">
      <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
          ...
          <div class="row">
            <div class="form-group col">
              <label class="text-color-dark text-3"> 비밀번호</label>
              <input type="password" v-model="editMember.mbrPswrd" class="form-control form-control-lg text-4" required :placeholder="editMember.mbrPswrd" :disabled="!editMode" @input="validatePswrd">
              <div v-if="errorMbrPswrd" class="error">{{ errorMbrPswrd }}</div>
            </div>
          </div>
...
const validatePswrd = () => {
  errorMbrPswrd.value = '';
  const pswrdPattern = /^[A-Za-z0-9]{4,20}$/;
  if (!pswrdPattern.test(editMember.value.mbrPswrd)) {
    errorMbrPswrd.value = '비밀번호 형식이 올바르지 않습니다.';
  }
};
